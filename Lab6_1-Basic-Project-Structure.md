...
2. **Build Process**: อธิบายขั้นตอนการ build ของ ESP-IDF ใน Docker container ตั้งแต่ source code จนได้ binary
    1.เข้า Docker container → docker-compose exec esp32-dev bash
...
ไฟล์ .gitignore มีความสำคัญมากสำหรับการพัฒนาโปรเจกต์ ESP32 เพราะช่วย
ป้องกันไม่ให้ไฟล์ที่สร้างอัตโนมัติและไฟล์ชั่วคราว เช่น ไฟล์ build (*.bin, build/), ไฟล์ config เฉพาะเครื่อง (sdkconfig.old), หรือไฟล์ editor settings (.vscode/) ถูกเพิ่มเข้าสู่ Git repository
...
ช่วยให้การ clone โปรเจกต์ไปเครื่องอื่นทำได้ง่ายและรวดเร็ว โดยไม่ต้องจัดการลบไฟล์ที่ไม่จำเป็น
...
Docker	                           Native System
+ ติดตั้งง่าย ไม่ยุ่งยาก	                 + ประสิทธิภาพสูงกว่าเล็กน้อย
+ ลดปัญหา environment ต่างกันในทีม    	 + เข้าถึงฮาร์ดแวร์และ debugging สะดวก
+ เหมาะกับ automation, CI/CD	       + เหมาะกับการพัฒนาระยะยาวและ debug ละเอียด
– ต้องตั้งค่า USB passthrough	         – อาจเจอปัญหาการติดตั้ง dependency
– อาจ debug ยากกว่าในบางกรณี	         – ต้องดูแล environment ให้ดี
